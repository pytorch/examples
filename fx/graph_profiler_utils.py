...
class NodeInfo:
    r"""
    The base class to store the profiling and static graph analysis information for all 
    ...
    7) in_peak_interval (bool): flags is True if the node lies in the peak memory interval
                                (when active memory exceeds device memory limit)
    8) total_peak_mem (int): This is the peak memory consumption calculated by simulating 
                            the memory usage for low memory mode.
    9) first_forward_access (Node): Reference to the node that first uses self in the 
                                    forward pass. Generally populated for the parameter
                                    nodes of the model.
    10) last_forward_access (Node): Reference to the node that last uses self in the 
                                    forward pass. Generally populated for the intermediate
                                    (activation) nodes.
    11) first_back_access (Node): Reference to the node that first uses self in the 
                                    backward pass. Generally populated for the intermediate
                                    (activation) nodes.
    12) last_back_access (Node): Reference to the node that last uses self in the 
                                backward pass. Generally populated for the intermediate 
                                (activation) nodes.
    """
    def __init__(self):
...
class IntNodeInfo(NodeInfo):
    r"""
    Derieved class to store the profiling and static graph analysis information for 
    intermediate nodes (activations) in the graph.
    1) idle_time (float): The idle time is calculated as 
                    [(last_backward_acess - swap_time) - (last_forward_access + swap_time)].
