...
def get_args():
    ...
    parser.add_argument(
        "--epochs", type=int, default=50, help="the number of total epochs to run."
    )
    parser.add_argument(
        "--batch_size", type=int, default=128, help="batch size. (default: 128)"
    )
    parser.add_argument(
        "--d_embed", type=int, default=100, help="the size of each embedding vector."
    )
    parser.add_argument(
        "--d_proj", type=int, default=300, help="the size of each projection layer."
    )
    parser.add_argument(
        "--d_hidden",
        type=int,
    ...
    parser.add_argument(
        "--log_every", type=int, default=50, help="iteration period to output log."
    )
    parser.add_argument(
        "--lr", type=float, default=0.001, help="initial learning rate."
    )
    parser.add_argument(
        "--dev_every", type=int, default=1000, help="log period of validation results."
    )
    parser.add_argument(
        "--save_every", type=int, default=1000, help="model checkpoint period."
    )
    parser.add_argument(
        "--dp_ratio",
        type=int,
    ...
    parser.add_argument(
        "--preserve-case", action="store_false", dest="lower", help="case-sensitivity."
    )
    parser.add_argument(
        "--no-projection",
        action="store_false",
    ...
    parser.add_argument(
        "--save_path", type=str, default="results", help="save path of results."
    )
    parser.add_argument(
        "--vector_cache",
        type=str,
    ...
    parser.add_argument(
        "--resume_snapshot", type=str, default="", help="model snapshot to resume."
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="run only a few iterations"
    )
