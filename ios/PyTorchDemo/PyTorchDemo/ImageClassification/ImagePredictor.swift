...
class ImagePredictor : NSObject {
    ...
    func predict_(_ buffer: UnsafeMutablePointer<Float32>, completionHandler:(([InferenceResult]?, Error?)->Void)){
        if isRunning {
            return
        }
        isRunning = true
        guard let inputTensor = TorchTensor.new(with: .float, size: [1,3,224,224], data: buffer) else {
            completionHandler([],ImagePredictorError.invalidInputTensor)
            return
        }
        let inputValue = TorchIValue.new(with: inputTensor)
